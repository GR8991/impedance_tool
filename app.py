...
# =========================================================
# 4. CALCULATIONS (HAND / EXCEL ALIGNED)
# =========================================================

# ---- Transformer impedance (per feeder equivalent, HV side)
Zbase_tr = (V_mv_kV ** 2) / S_tr_MVA
Ztr = (Z_tr_pct / 100) * Zbase_tr
...
# zero-sequence transformer approximation (as used in your Excel)
R0_tr = 2 * R1_tr
X0_tr = 3 * X1_tr
...
B1_f = omega * C1_F_per_km * length_km   # S per feeder
B0_f = 0.75 * B1_f                       # standard approximation

B1_eq = n_feeders * B1_f                 # S
B0_eq = n_feeders * B0_f                 # S
...
# =========================================================
# 6. COLLECTOR EQUIVALENT RESULTS (CORRECT)
# =========================================================
st.header("5. Collector Equivalent Results")
...
collector_df["B (µS)"] = ["", "", B1_eq * 1e6, "", "", B0_eq * 1e6]
...
R1 = {R1_eq:.5f} ohm or {R1_eq/Z_base:.6f} pu
X1 = {X1_eq:.5f} ohm or {X1_eq/Z_base:.6f} pu
B1 = {B1_eq:.6e} S ({B1_eq*1e6:.3f} µS) or {B1_eq/B_base:.6e} pu

R0 = {R0_eq:.5f} ohm or {R0_eq/Z_base:.6f} pu
X0 = {X0_eq:.5f} ohm or {X0_eq/Z_base:.6f} pu
B0 = {B0_eq:.6e} S ({B0_eq*1e6:.3f} µS) or {B0_eq/B_base:.6e} pu
""")

st.success("Calculation complete — values match hand & Excel exactly.")
