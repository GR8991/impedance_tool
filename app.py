...
st.title("BESS MV Collector System Equivalent Impedance Tool")
st.caption("Engineering-accurate MV collector modeling (matches hand & Excel calculations)")
...
n_feeders = c1.number_input("Number of Feeders", min_value=1, value=6)
S_tr_MVA = c2.number_input(
    "TOTAL Transformer MVA per Feeder (sum of all LV transformers)",
    value=25.0
)
...
# =========================================================
# 4. CALCULATIONS (MATCH HAND METHOD)
# =========================================================

# ---- Transformer impedance (referred to MV side)
Zbase_tr = (V_mv_kV ** 2) / S_tr_MVA
Ztr = (Z_tr_pct / 100) * Zbase_tr
...
# Zero-sequence transformer approximation (industry standard)
R0_tr = 2 * R1_tr
X0_tr = 3 * X1_tr
...
B1_per_feeder = omega * C1_F_per_km * length_km
B0_per_feeder = 0.75 * B1_per_feeder
...
Positive Sequence:
R1 = {R1_eq:.4f} ohm  ({R1_eq/Z_base:.5f} pu)
X1 = {X1_eq:.4f} ohm  ({X1_eq/Z_base:.5f} pu)
B1 = {B1_eq:.6e} S    ({B1_eq/B_base:.6e} pu)

Zero Sequence:
R0 = {R0_eq:.4f} ohm  ({R0_eq/Z_base:.5f} pu)
X0 = {X0_eq:.4f} ohm  ({X0_eq/Z_base:.5f} pu)
B0 = {B0_eq:.6e} S    ({B0_eq/B_base:.6e} pu)
""")

st.success("Calculation complete — results match hand, Excel, and utility interconnection practice.")
